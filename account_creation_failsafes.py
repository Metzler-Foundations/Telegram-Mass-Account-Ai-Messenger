...
class AccountCreationFailSafe:
    ...
    def check_rate_limits(self, phone_number: str, ip_address: str) -> Tuple[bool, Optional[str]]:
        ...
        recent_creations = [
            c for c in self.creation_history
            if c.get('timestamp', datetime.min) > hour_ago
        ]
        if len(recent_creations) >= self.rate_limits['per_hour']:
            return False, f"Hourly limit exceeded: {len(recent_creations)}/{self.rate_limits['per_hour']}"
        
        # Check per-day limit
        daily_creations = [
            c for c in self.creation_history
            if c.get('timestamp', datetime.min) > day_ago
        ]
        if len(daily_creations) >= self.rate_limits['per_day']:
            return False, f"Daily limit exceeded: {len(daily_creations)}/{self.rate_limits['per_day']}"
        
        # Check per-IP limit
        ip_creations = [
            c for c in recent_creations
            if c.get('ip_address') == ip_address
        ]
        if len(ip_creations) >= self.rate_limits['per_ip']:
            return False, f"IP limit exceeded: {len(ip_creations)}/{self.rate_limits['per_ip']} for {ip_address}"
    ...
    def check_failure_patterns(self, config: Dict) -> Tuple[bool, Optional[str]]:
        ...
        recent_failures = [
            c for c in self.creation_history[-10:]
    ...
    def record_creation_attempt(self, phone_number: str, ip_address: str, 
    ...
    def _update_failure_patterns(self, provider: str, error: Optional[str]):
        ...
        if pattern['total_attempts'] >= 5:
            ...
            if failure_rate > 0.8:  # 80% failure rate
                ...
                logger.warning(f"Blocked phone provider {provider} due to high failure rate: {failure_rate:.1%}")
    
    async def pre_creation_check(self, config: Dict, phone_number: str, 
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            ) if self.creation_history else 0.0
        }
