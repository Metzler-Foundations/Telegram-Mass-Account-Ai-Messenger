...
class CostReconciliation:
    ...
    def get_tracked_costs(self, provider: str, start_date: str, end_date: str) -> float:
        ...
        try:
            ...
            with pool.get_connection() as conn:
                ...
                """, (provider, start_date, end_date)).fetchone()
    ...
    def reconcile(self, provider: str, actual_charge: float, 
        ...
        try:
            ...
            with pool.get_connection() as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    record['provider'], record['period'], record['tracked_cost'],
                    record['actual_charge'], record['difference'],
                    record['variance_percentage'], record['status'],
...
def reconcile_provider_costs(provider: str, actual_charge: float, 
                            start_date: str, end_date: str) -> Dict:
