...
class ConversationAnalyzer:
    ...
    def _prune_old_flows(self):
        ...
        try:
            ...
            if stale_ids:
                cursor.executemany(
                    "DELETE FROM conversation_stages WHERE flow_id = ?",
                    [(fid,) for fid in stale_ids],
                )
                cursor.executemany(
                    "DELETE FROM conversions WHERE flow_id = ?", [(fid,) for fid in stale_ids]
                )
                cursor.executemany(
                    "DELETE FROM conversation_flows WHERE flow_id = ?",
                    [(fid,) for fid in stale_ids],
                )
                conn.commit()
    ...
    def analyze_conversation(
        ...
        self._save_conversation_flow(
            flow_id, user_id, messages, responses, outcome, value, response_times, timestamps
        )
    ...
    def _extract_patterns(
        self, messages: List[str], responses: List[str], outcome: str
    ) -> List[MessagePattern]:
    ...
    def _save_conversation_flow(
        ...
        for i, (msg, resp) in enumerate(zip(messages, responses)):
            ...
            sent_at, responded_at = (
                timestamps[i] if timestamps and i < len(timestamps) else (None, None)
            )
    ...
    def _update_pattern(self, pattern: MessagePattern):
        ...
        cursor.execute(
            "SELECT times_used, success_count FROM message_patterns WHERE pattern_id = ?",
            (pattern.pattern_id,),
        )
        row = cursor.fetchone()
    ...
    def get_conversion_insights(self) -> Dict:
        ...
        best_flows = []
        for row in cursor.fetchall():
            best_flows.append({"messages": json.loads(row[0]) if row[0] else [], "value": row[1]})
