...
class MigrationManager:
    ...
    def _apply_migration(self, migration: Migration) -> bool:
        ...
        try:
            ...
            cursor.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (migration.version, migration.name, self._calculate_checksum(migration), "system"),
            )
    ...
    def rollback(self, target_version: int) -> bool:
        ...
        to_rollback = [m for m in self.migrations if target_version < m.version <= current_version]
        to_rollback.reverse()
...
def setup_migrations(manager: MigrationManager):
    ...
    manager.register_migration(
        Migration(version=1, name="add_unique_constraints", up=migration_001_add_unique_constraints)
    )

    manager.register_migration(
        Migration(version=2, name="add_idempotency_keys", up=migration_002_add_idempotency_keys)
    )
