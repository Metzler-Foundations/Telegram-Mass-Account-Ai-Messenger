...
def migrate_proxy_database(db_path: str = "proxy_pool.db"):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            indexes_to_create = [
                ("idx_proxy_fraud_score", "CREATE INDEX IF NOT EXISTS idx_proxy_fraud_score ON proxies(fraud_score)"),
                ("idx_proxy_last_check", "CREATE INDEX IF NOT EXISTS idx_proxy_last_check ON proxies(last_check)"),
                ("idx_proxy_status_score", "CREATE INDEX IF NOT EXISTS idx_proxy_status_score ON proxies(status, score DESC)"),
            ]
