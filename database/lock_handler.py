...
class DatabaseLockHandler:
            ...
            try:
                with self.get_connection(db_path, read_only=read_only) as conn:
                    ...
                    if attempt > 0:
                        ...
                        logger.info(f"Database operation succeeded after {attempt + 1} attempts ({wait_time:.2f}s)")
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            **self._lock_statistics,
            'avg_wait_time': (
                self._lock_statistics['total_wait_time'] / self._lock_statistics['retries_succeeded']
