...
class ConfigValidator:
    ...
    SCHEMA = {
        ...
        "gemini": {
            "required": True,
            "type": "object",
            "properties": {"api_key": {"type": ["string", "null"], "required": False}},
        },
        "sms_providers": {
            "required": True,
            "type": "object",
    ...
    def validate(config: Dict[str, Any]) -> List[str]:
        ...
        for section, schema in ConfigValidator.SCHEMA.items():
            ...
            if section in config:
                section_errors = ConfigValidator._validate_section(config[section], schema, section)
                errors.extend(section_errors)

        return errors

    @staticmethod
    def _validate_section(value: Any, schema: Dict, path: str) -> List[str]:
        ...
        if expected_types:
            ...
            if not type_valid:
                errors.append(
                    f"{path}: Invalid type (expected {expected_types}, got {type(value).__name__})"
                )
                return errors
...
class ConfigManager:
    ...
    def load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            config = safe_json_load_file(str(self.config_path), default={}, raise_on_error=True)
