        ...
        if config.give_up_on_exceptions:
            for exc_type in config.give_up_on_exceptions:
                if isinstance(exception, exc_type):
                    logger.debug(
                        f"Not retrying: exception type {exc_type.__name__} in give_up list"
                    )
                    return False
        ...
        if config.retry_on_exceptions:
            for exc_type in config.retry_on_exceptions:
                if isinstance(exception, exc_type):
                    return True
            logger.debug(
                f"Not retrying: exception type {type(exception).__name__} not in retry list"
            )
            return False
    ...
    async def retry_async(
        ...
        for attempt in range(config.max_attempts):
            try:
                ...
                if progress_callback:
                    progress_callback(
                        attempt + 1, config.max_attempts, f"⏳ Waiting {delay:.0f}s before retry..."
                    )
...
async def retry_async(
    func: Callable, max_attempts: int = 3, base_delay: float = 1.0, context: str = ""
) -> Any:
