...
class JSONValidator:
    ...
    def validate_json_string(json_str: str, max_size: Optional[int] = None) -> bool:
        ...
        if len(json_str) > max_size:
            raise JSONParseError(f"JSON too large ({len(json_str)} bytes > {max_size} bytes)")
...
def safe_json_loads(
    json_str: str, default: Any = None, max_size: Optional[int] = None, raise_on_error: bool = False
) -> Any:
    """
...
def safe_json_dumps(
    obj: Any, default: str = "{}", indent: Optional[int] = None, raise_on_error: bool = False
) -> str:
    """
