...
class PersistentConnectionManager:
        ...
        while self.is_monitoring:
            try:
                for phone_number, client_info in list(self.monitored_clients.items()):
                    ...
                    if not is_connected:
                        logger.warning(f"⚠️ Account {phone_number} disconnected! Attempting immediate reconnect...")
    ...
    async def register_client(self, phone_number: str, client: Client, 
    ...
    async def _reconnect_client(self, phone_number: str, client_info: Dict):
        ...
        if reconnect_attempts >= self.max_reconnect_attempts:
            logger.error(f"❌ Max reconnect attempts reached for {phone_number}. Manual intervention required.")
        ...
        logger.info(f"🔄 Reconnecting {phone_number} (attempt {reconnect_attempts + 1}/{self.max_reconnect_attempts}) in {delay}s...")
        ...
        try:
            ...
            logger.error(f"❌ Reconnection attempt {reconnect_attempts + 1} failed for {phone_number}: {e}")
    ...
    def get_all_statuses(self) -> Dict[str, Dict]:
        ...
        return {
            phone: self.get_connection_status(phone)
            for phone in self.monitored_clients.keys()
        }
