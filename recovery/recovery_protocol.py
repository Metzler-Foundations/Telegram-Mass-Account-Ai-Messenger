...
class RecoveryProtocol:
    ...
    def initiate_recovery(self, account_id: str, severity: str = "moderate") -> RecoveryPlan:
        ...
        self._log_action(account_id, "recovery_initiated", plan.stage, 
                        f"Severity: {severity}")
    ...
    def _save_plan(self, plan: RecoveryPlan):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (plan.account_id, plan.stage.value, plan.cooldown_until,
              plan.activity_limit, plan.started_at, plan.estimated_recovery,
              datetime.now()))
        conn.commit()
        conn.close()
