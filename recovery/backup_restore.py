...
class BackupManagerDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.include_sessions_check.setToolTip(
            "Session files contain login credentials - backup for safety"
        )
    ...
    def load_backups(self):
        ...
        for backup_file in sorted(backup_files, reverse=True):
            ...
            item_text = (
                f"{backup_file.name} ({size_mb:.2f} MB) - {modified.strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    def create_backup(self):
        ...
        self.backup_thread = BackupCreator(file_path, self.include_sessions_check.isChecked())
...
def create_auto_backup(include_sessions: bool = True) -> Optional[str]:
    ...
    try:
        ...
        files_to_backup = ["config.json", "members.db", "campaigns.db", "warmup_jobs.json"]
...
def cleanup_old_backups(backup_dir: Path, keep: int = 10):
    ...
    try:
        backups = sorted(
            backup_dir.glob("auto_backup_*.zip"), key=lambda p: p.stat().st_mtime, reverse=True
        )
