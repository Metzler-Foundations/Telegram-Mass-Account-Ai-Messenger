...
class AutoIntegrator:
    ...
    async def enhance_campaign_targets(
        self,
        client,
        ...
        try:
            ...
            qualified_users = [
                intel for intel in intel_results if intel.value_score >= min_value_score
            ]
            ...
            logger.info(
                f"✅ Campaign enhanced: {len(qualified_users)} high-value targets prioritized"
            )
        ...
        try:
            ...
            if self.features.status_tracker:
                self.features.status_tracker.track_message_sent(
                    message_id=0, chat_id=user_id, user_id=user_id  # Will be set by actual send
                )
