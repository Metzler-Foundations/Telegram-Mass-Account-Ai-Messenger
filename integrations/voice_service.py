...
class VoiceConfig:
    ...
    def __post_init__(self):
        if self.keywords is None:
            self.keywords = ["hey", "hi", "hello", "interested", "price", "how much", "tell me more", "😊", "❤️"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "trigger_mode": self.trigger_mode.value if isinstance(self.trigger_mode, VoiceTriggerMode) else self.trigger_mode,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'VoiceConfig':
        ...
        return cls(
            ...
            keywords=data.get("keywords", ["hey", "hi", "hello", "interested", "price", "how much"]),
...
class VoiceService:
    ...
    def _check_rate_limit(self, chat_id: int = None) -> Tuple[bool, str]:
        ...
        if chat_id is not None:
            ...
            if len(self.voice_timestamps_by_chat[chat_id]) >= MAX_VOICE_MESSAGES_PER_CHAT_PER_HOUR:
                return False, f"Chat rate limit reached ({MAX_VOICE_MESSAGES_PER_CHAT_PER_HOUR}/hour per chat)"
    ...
    async def generate_voice_message(self, text: str, voice_id: str = None, 
                                      use_cache: bool = True,
                                      chat_id: int = None) -> Optional[Path]:
        ...
        for attempt, try_voice_id in enumerate(voices_to_try[:self.max_retries]):
            try:
                ...
                logger.info(f"Generating voice message with voice '{try_profile.name}' "
        ...
        logger.error(f"Failed to generate voice message after {self.max_retries} attempts: {last_error}")
        self.analytics.record_failed()
        return None
    ...
    def should_send_voice(self, chat_id: int, message_text: str, 
                          config: VoiceConfig) -> bool:
        ...
        if depth < config.min_messages_before_voice:
            logger.debug(f"Skipping voice - conversation too new ({depth}/{config.min_messages_before_voice})")
            return False
        ...
        if mode == VoiceTriggerMode.SMART:
            ...
            interest_keywords = ["interested", "tell me", "want", "like", "love", "price", "how much", "😊", "❤️", "🔥"]
    ...
    async def test_voice_generation(self, voice_id: str = None, 
                                     phrase_type: str = "greeting") -> Tuple[bool, Optional[Path]]:
