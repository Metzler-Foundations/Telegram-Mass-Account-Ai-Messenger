...
class AccountHealthDashboard(QWidget):
            ...
            for phone in self.account_manager.accounts.keys():
                ...
                if self.anti_detection:
                    try:
                        ...
                        account_data['message_diversity'] = ad_status.get('message_diversity_score', 1.0)
    ...
    def _release_account(self, phone: str):
        ...
        if self.anti_detection:
            try:
                ...
                ErrorHandler.safe_information(self, "Success", f"Account {phone} released from quarantine.")
                self.refresh_data()
            except Exception as e:
    ...
    def _show_account_details(self, phone: str):
        ...
        ErrorHandler.safe_information(self, "Account Details", f"Showing details for {phone}\n(Full dialog would be implemented here)")
    ...
    def pause_all_at_risk(self):
        ...
        if not at_risk:
            ErrorHandler.safe_information(self, "No At-Risk Accounts", "No accounts are currently at high risk.")
            return
        
        if ErrorHandler.safe_question(
            self, "Confirm Pause",
            f"This will pause {len(at_risk)} at-risk accounts. Continue?"
        ):
    ...
    def release_all_quarantined(self):
        ...
        if not quarantined:
            ErrorHandler.safe_information(self, "No Quarantined Accounts", "No accounts are currently quarantined.")
            return
