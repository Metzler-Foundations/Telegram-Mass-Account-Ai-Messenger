...
def test_wizard_manager():
    ...
    assert manager.get_starting_step() == SetupWizardManager.STEP_TELEGRAM, "Should start at Telegram step"
    ...
    assert manager.get_starting_step() == SetupWizardManager.STEP_GEMINI, "Should start at Gemini step"
    ...
    telegram_gemini = {
        ...
        "gemini": {
            "api_key": "AIzaSyABC123XYZ_TEST_KEY_LONG_ENOUGH_TO_PASS"
        }
    }
    manager = SetupWizardManager(telegram_gemini)
    assert manager.is_wizard_needed() == True, "Wizard should be needed when SMS provider is missing"
    assert manager.get_starting_step() == SetupWizardManager.STEP_SMS_PROVIDER, "Should start at SMS Provider step"
    ...
    is_valid, errors = manager.is_step_complete(SetupWizardManager.STEP_SMS_PROVIDER, complete_settings)
