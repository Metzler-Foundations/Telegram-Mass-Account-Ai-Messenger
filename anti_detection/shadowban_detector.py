...
class ShadowBanDetector:
    ...
    async def test_delivery(
        self, client: Client, account_id: str, canary_user_id: int
    ) -> DeliveryTest:
    ...
    def _save_test_result(self, test: DeliveryTest):
        ...
        """,
            (
                ...
                test.account_id,
                test.canary_id,
                int(test.message_sent),
                int(test.message_received),
                test.delay_seconds,
                test.timestamp,
            ),
        )
    ...
    def _update_account_status(self, account_id: str, test: DeliveryTest):
        ...
        if status != ShadowBanStatus.CLEAR:
            logger.warning(
                f"Shadow ban detected for {account_id}: {status.value} (rate: {delivery_rate:.2%})"
            )
