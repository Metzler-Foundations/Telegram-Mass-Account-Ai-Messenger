...
from pyrogram.errors import (
    ...
    ChannelPrivate, ChatAdminRequired, PeerIdInvalid
)
...
class GroupDiscoveryEngine:
    ...
    def _init_database(self):
        ...
        # Indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_group_quality ON discovered_groups(quality_score DESC)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_group_method ON discovered_groups(discovery_method)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_group_joined ON discovered_groups(is_joined)")
    ...
    async def discover_nearby_groups(self, client: Client, 
                                    location: LocationProfile,
                                    max_groups: int = 50) -> List[DiscoveredGroup]:
        ...
        try:
            ...
            self._log_discovery(DiscoveryMethod.NEARBY, len(discovered), True, {
    ...
    async def discover_from_common_groups(self, client: Client, 
                                         user_ids: List[int],
                                         max_per_user: int = 10) -> List[DiscoveredGroup]:
        ...
        self._log_discovery(DiscoveryMethod.COMMON_GROUPS, len(discovered), True, {
    ...
    async def validate_invite_link(self, client: Client, invite_link: str) -> Optional[DiscoveredGroup]:
    ...
    async def analyze_group_events(self, client: Client, group_id: int,
    ...
    async def _process_discovered_group(self, client: Client, chat: Chat,
                                       method: DiscoveryMethod,
                                       source_id: Optional[int] = None,
                                       location: Optional[LocationProfile] = None) -> Optional[DiscoveredGroup]:
    ...
    def _calculate_quality_score(self, group: DiscoveredGroup) -> float:
        ...
        spam_keywords = ['free', 'download', 'movie', 'xxx', 'porn', '18+', 
                        'casino', 'bet', 'earn', 'money', 'prize']
    ...
    def _save_group(self, group: DiscoveredGroup):
        ...
        """, (
            group.group_id, group.title, group.username, group.invite_link,
            group.discovery_method.value, group.discovered_at, group.discovered_from,
            group.member_count, group.online_count, group.description,
            int(group.is_verified), int(group.is_scam), int(group.is_fake),
            group.latitude, group.longitude, group.distance,
            group.quality_score, group.quality_level.value,
            json.dumps(group.spam_indicators),
            group.messages_per_day, group.activity_score,
            int(group.is_joined), 
            group.joined_at.isoformat() if group.joined_at else None,
            group.join_attempts, group.last_updated
        ))
    ...
    def _log_discovery(self, method: DiscoveryMethod, groups_found: int, 
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (method.value, groups_found, datetime.now(), 
              json.dumps(params) if params else None, int(success)))
        conn.commit()
        conn.close()
    
    def get_top_quality_groups(self, limit: int = 50, 
                              min_score: float = 60.0,
                              not_joined_only: bool = False) -> List[DiscoveredGroup]:
