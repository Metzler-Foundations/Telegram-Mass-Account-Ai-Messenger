...
class RelationshipMapper:
    ...
    def _init_database(self):
        ...
        # Indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_rel_strength ON relationships(strength DESC)")
    ...
    def add_relationship(self, user_id_1: int, user_id_2: int, 
                        interaction_type: str = "message", weight: float = 1.0):
        ...
        try:
            ...
            for i, comm in enumerate(communities):
                if len(comm) >= min_size:
                    ...
                    cluster.central_members = sorted(centrality, key=centrality.get, reverse=True)[:5]
    ...
    def _save_cluster(self, cluster: CommunityCluster):
        ...
        """, (cluster.cluster_id, cluster.density, cluster.avg_connections, 
              json.dumps(cluster.topics), now, now))
    ...
    def rebuild_graph_from_db(self):
        ...
        conn.close()
        logger.info(f"Rebuilt graph with {self.graph.number_of_nodes()} nodes and {self.graph.number_of_edges()} edges")
