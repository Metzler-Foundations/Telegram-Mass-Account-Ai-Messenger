...
class VariantStatistics:
    ...
    def calculate_chi_square(
        ...
        if any(e == 0 for e in [expected_a_success, expected_a_failure, expected_b_success, expected_b_failure]):
            return 0.0, 1.0
    ...
    def test_variants(
        ...
        chi_square, p_value = VariantStatistics.calculate_chi_square(
            variant_a_sent, variant_a_success,
            variant_b_sent, variant_b_success
        )
        ...
        sample_size_adequate = (
            variant_a_sent >= recommended_min and 
            variant_b_sent >= recommended_min
        )
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                for row in cursor:
                    variants.append({
                        ...
                        'rate': row['success_count'] / row['total_sent'] if row['total_sent'] > 0 else 0
                    })
    ...
    @staticmethod
    def test_campaign_variants(campaign_id: int, db_path: str = "campaigns.db") -> List[VariantTestResult]:
        """
