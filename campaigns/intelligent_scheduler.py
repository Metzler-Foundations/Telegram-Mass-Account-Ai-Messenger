...
class IntelligentScheduler:
    ...
    def __init__(self, db_path: str = "scheduler.db", status_db: str = "status_intelligence.db"):
    ...
    def schedule_optimal(self, user_id: int, message: str, timezone: str = "UTC") -> datetime:
        ...
        if not allowed_hours:
            allowed_hours = [
                h for h in range(0, 24) if not self._is_blackout_hour(h)
            ] or online_hours
        ...
        scheduled = now.replace(
            hour=best_hour, minute=random.randint(0, 59), second=0, microsecond=0
        )
    ...
    def _save_scheduled_message(
        self, user_id: int, message: str, scheduled_time: datetime, timezone: str
    ):
    ...
    def get_due_messages(self) -> List[Dict]:
        ...
        for row in cursor.fetchall():
            messages.append({"id": row[0], "user_id": row[1], "message": row[2]})
    ...
    def cleanup_stale_records(
        self, sent_retention_days: int = 30, pending_retention_days: int = 14
    ):
