    ...
    def record_message_sent(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (message_id, campaign_id, user_id, account_phone, sent_at, DeliveryStatus.SENT.value))
                conn.commit()
    ...
    def record_delivery(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                row = cursor.fetchone()
                if not row:
                    logger.warning(f"No sent record found for campaign {campaign_id}, user {user_id}")
                    return False
                ...
                conn.execute('''
                    ...
                    SET delivered_at = ?, 
                        ...
                        updated_at = CURRENT_TIMESTAMP
                    WHERE campaign_id = ? AND user_id = ?
                ''', (delivered_at, delivery_time, DeliveryStatus.DELIVERED.value, campaign_id, user_id))
    ...
    def _update_campaign_stats(self, campaign_id: int):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (
                    campaign_id, total_sent, total_delivered, total_read, total_replied,
                    row[4], row[5], row[6], median_response,
                    delivery_rate, read_rate, response_rate,
                    datetime.now()
                ))
