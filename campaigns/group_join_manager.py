...
class GroupJoinManager:
    ...
    def __init__(self, cooldown_seconds: int = 300, max_joins_per_hour: int = 10, max_joins_per_day: int = 50):
    ...
    async def can_join_group(self, account_id: str) -> tuple[bool, float]:
        ...
        async with self.lock:
            ...
            if hourly_count >= self.max_joins_per_hour:
                logger.warning(f"Account {account_id} hit hourly limit: {hourly_count}/{self.max_joins_per_hour}")
            ...
            if daily_count >= self.max_joins_per_day:
                logger.warning(f"Account {account_id} hit daily limit: {daily_count}/{self.max_joins_per_day}")
                # Calculate time until oldest join expires
    ...
    def _clean_old_timestamps(self, account_id: str, now: datetime):
        ...
        if account_id in self.hourly_joins:
            self.hourly_joins[account_id] = [
                ts for ts in self.hourly_joins[account_id]
        ...
        if account_id in self.daily_joins:
            self.daily_joins[account_id] = [
                ts for ts in self.daily_joins[account_id]
    ...
    async def record_join(self, account_id: str):
        ...
        async with self.lock:
            ...
            logger.debug(f"Recorded group join for {account_id} (hourly: {len(self.hourly_joins[account_id])}/{self.max_joins_per_hour}, daily: {len(self.daily_joins[account_id])}/{self.max_joins_per_day})")
