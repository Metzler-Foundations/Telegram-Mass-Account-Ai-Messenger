...
class ProxyMonitorWidget(QWidget):
    ...
    def on_all_tests_complete(self):
        ...
        degraded = sum(
            1 for r in self.test_results.values() if r["status"] in ["degraded", "timeout"]
        )
    ...
    def remove_failed_proxies(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            QMessageBox.information(self, "Success", f"Removed {failed_count} failed proxies")
    ...
    def update_status(self):
        ...
        healthy = sum(1 for r in self.test_results.values() if r.get("status") == "healthy")
        ...
        if tested == 0:
            ...
            self.status_label.setText(f"📊 {healthy}/{tested} proxies healthy")
