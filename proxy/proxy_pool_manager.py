...
class ProxyCredentialManager:
    ...
    def _init_cipher(self):
        ...
        if not CRYPTOGRAPHY_AVAILABLE:
            logger.warning(
                "Cryptography not available - proxy credentials will be stored in plain text"
            )
            return
    ...
    def _get_or_create_encryption_key(self) -> bytes:
        ...
        try:
            ...
            keyring.set_password(
                keyring_service, "encryption_key", base64.urlsafe_b64encode(key).decode()
            )
...
class Proxy:
    ...
    def pyrogram_dict(self) -> Dict:
        ...
        proxy_dict = {"scheme": self.protocol.value, "hostname": self.ip, "port": self.port}
        if self.username:
...
class ProxyPoolManager:
    ...
    def __init__(self, db_path: str = "proxy_pool.db", config_path: str = "config.json"):
        ...
        self.endpoints: List[ProxyEndpoint] = (
            self.PRIMARY_ENDPOINTS + self.SECONDARY_ENDPOINTS + self.OBSCURE_ENDPOINTS
        )
        ...
        if not self._verify_database_integrity():
            logger.warning(
                "Proxy database integrity check failed; writes may be unsafe until repaired"
            )
        else:
            ...
            if config_file.exists():
                ...
                if "proxy_pool" in file_config:
                    ...
                    if "min_score_threshold" in proxy_pool_config:
                        self.config["min_score_threshold"] = proxy_pool_config[
                            "min_score_threshold"
                        ]
                    ...
                    if "auto_cleanup_enabled" in proxy_pool_config:
                        self.config["auto_cleanup_enabled"] = proxy_pool_config[
                            "auto_cleanup_enabled"
                        ]
                    if "health_check_batch_size" in proxy_pool_config:
                        self.config["health_check_batch_size"] = proxy_pool_config[
                            "health_check_batch_size"
                        ]
                    if "health_check_interval" in proxy_pool_config:
                        self.config["health_check_interval"] = proxy_pool_config[
    ...
    def _init_database(self):
        ...
        with self._get_connection() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_proxy_assigned ON proxies(assigned_account)"
            )
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_proxy_last_used ON proxies(last_used DESC)"
            )
            ...
            with self._get_connection() as conn:
                conn.execute(
                    ...
                    (
                        ...
                        (
                            self.stats["last_full_poll"].isoformat()
                            if self.stats["last_full_poll"]
                            else None
                        ),
                    ),
                )
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    decrypted_username = (
                        self.credential_manager.decrypt_credential(row["username"])
                        if row["username"]
                        else None
                    )
                    decrypted_password = (
                        self.credential_manager.decrypt_credential(row["password"])
                        if row["password"]
                        else None
                    )
    ...
    def _save_proxy(self, proxy: Proxy):
        ...
        try:
            with self._get_connection() as conn:
                ...
                encrypted_username = (
                    self.credential_manager.encrypt_credential(proxy.username)
                    if proxy.username
                    else None
                )
                encrypted_password = (
                    self.credential_manager.encrypt_credential(proxy.password)
                    if proxy.password
                    else None
                )
        ...
        try:
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if needs_encryption:
                        encrypted_username = (
                            self.credential_manager.encrypt_credential(username)
                            if username
                            else None
                        )
                        encrypted_password = (
                            self.credential_manager.encrypt_credential(password)
                            if password
                            else None
                        )

                        credentials_to_update.append(
                            (encrypted_username, encrypted_password, proxy_key)
                        )
    ...
    async def _run_health_checks(self):
        ...
        assigned_proxies = [
            ...
            if proxy.assigned_account
            and proxy.status in [ProxyStatus.ACTIVE, ProxyStatus.TESTING]
    ...
    def _log_health_check(
        self, proxy: Proxy, latency: float, success: bool, error_msg: Optional[str]
    ):
    ...
    async def _cleanup_low_quality_proxies(self):
        ...
        try:
            async with self._lock:
                ...
                unassigned_proxies = [
                    proxy for proxy in self.proxies.values() if not proxy.assigned_account
                ]
                ...
                for proxy in unassigned_proxies:
                    ...
                    if (
                        proxy.score < self.config["min_score_threshold"]
                        or proxy.fraud_score >= self.config["max_fraud_score"]
                        or proxy.status == ProxyStatus.FAILED
                    ):
    ...
    async def get_proxy_for_account(
        self, account_phone: str, prefer_tier: ProxyTier = ProxyTier.STANDARD
    ) -> Optional[Proxy]:
        """
    ...
    async def _assign_new_proxy(
        self, account_phone: str, prefer_tier: ProxyTier
    ) -> Optional[Proxy]:
        ...
        async with self._lock:
            ...
            available = [
                self.proxies[key]
                for key in self.available_proxies
                if key in self.proxies
                and self.proxies[key].score >= self.config["min_score"]
    ...
    async def release_proxy(self, account_phone: str):
        ...
        async with self._lock:
            ...
            try:
                with self._get_connection() as conn:
                    conn.execute(
                        "DELETE FROM proxy_assignments WHERE account_phone = ?", (account_phone,)
                    )
                    conn.commit()
            except Exception as e:
    ...
    def get_proxy_stats(self) -> Dict[str, Any]:
        ...
        avg_latency = (
            sum(p.latency_ms for p in self.proxies.values() if p.latency_ms > 0) / total
            if total > 0
            else 0
        )
            ...
            with self._get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if proxy_dict.get("username"):
                        proxy_dict["username"] = self.credential_manager.decrypt_credential(
                            proxy_dict["username"]
                        )
                    if proxy_dict.get("password"):
                        proxy_dict["password"] = self.credential_manager.decrypt_credential(
                            proxy_dict["password"]
                        )
