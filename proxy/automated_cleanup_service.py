...
class AutomatedProxyCleanupService:
    ...
    def __init__(self, proxy_pool_manager, db_path: str = "proxy_cleanup_audit.db"):
        ...
        # Statistics
        self.stats = {"total_cleaned": 0, "last_cleanup": None, "last_cleanup_count": 0}
    ...
    async def run_cleanup(self) -> Dict[str, Any]:
        ...
        try:
            ...
            proxies, _ = self.proxy_pool_manager.get_proxies_paginated(
                page=1, page_size=10000, assigned_only=False
            )
            ...
            return {
                ...
                "reasons": {
                    r.value: sum(1 for e in cleanup_events if e.reason == r) for r in CleanupReason
                },
    ...
    async def _remove_proxies(self, proxy_keys: List[str]):
        ...
        try:
            ...
            with sqlite3.connect(self.proxy_pool_manager.db_path) as conn:
                ...
                conn.executemany(
                    "DELETE FROM proxies WHERE proxy_key = ?", [(key,) for key in proxy_keys]
                )
                conn.commit()
