...
class PhoneNumberProvider:
    ...
    def get_number(self, provider: str, country: str, api_key: str) -> Optional[Dict]:
        ...
        retry_config = RetryConfig(
            max_attempts=3, base_delay=2.0, strategy=RetryStrategy.EXPONENTIAL, jitter=True
        )
    ...
    def check_inventory(
        self, provider: str, country: str, api_key: str, requested: int
    ) -> Dict[str, Any]:
        """Best-effort inventory probe to avoid starting runs without enough numbers."""
        if provider not in self.providers:
            return {"success": False, "error": f"Unsupported phone provider '{provider}'"}

        if not api_key:
            return {"success": False, "error": f"Phone provider API key is required for {provider}"}
        ...
        try:
            ...
            if provider == "smspool":
                available, warning = self._check_smspool_inventory(
                    country, api_key, provider_config
                )
                return {"success": True, "available": available, "warning": warning}

            if provider == "textverified":
                available, warning = self._check_textverified_inventory(
                    country, api_key, provider_config
                )
                return {"success": True, "available": available, "warning": warning}

            if provider == "daisysms":
                available, warning = self._check_daisysms_inventory(
                    country, api_key, provider_config
                )
                return {"success": True, "available": available, "warning": warning}
            ...
            return {"success": False, "error": f"Inventory check failed for {provider}: {exc}"}

    def _check_smspool_inventory(
        self, country: str, api_key: str, config: Dict
    ...
    def _check_textverified_inventory(
        self, country: str, api_key: str, config: Dict
    ) -> Tuple[Optional[int], Optional[str]]:
    ...
    def _check_daisysms_inventory(
        ...
        try:
            response = requests.get(
                ...
                params={
                    "api_key": api_key,
                    "service": config.get("service_code", "telegram"),
                    "country": country,
                },
                timeout=8,
            )
        ...
        try:
            ...
            if response.status_code == 201:  # Created
                ...
                if result.get("id") and result.get("number"):
                    ...
                    logger.warning(
                        f"TextVerified API error: {result.get('error', 'Unknown error')}"
                    )
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if data.get("status") == "success" and data.get("number"):
                        ...
                        logger.warning(
                            f"SMS-Hub API error (JSON): {data.get('message', 'Unknown error')}"
                        )
                except json.JSONDecodeError:
                ...
                if products:
                    ...
                    if isinstance(products, dict):
                        ...
                        for name, details in products.items():
                            quantity = (
                                details.get("Qty") or details.get("qty") or details.get("count", 0)
                            )
                    ...
                    if buy_response.status_code == 200:
                        ...
                        if result.get("phone"):
                            ...
                            logger.warning(
                                f"5SIM buy error: {result.get('error', 'Unknown error')}"
                            )
                    else:
                        logger.warning(
                            f"5SIM buy failed HTTP {buy_response.status_code}: {buy_response.text}"
                        )
    ...
    def cancel_number(self, provider: str, number_id: str, api_key: str) -> bool:
        ...
        try:
            ...
            if provider == "sms-hub":
                params = {"api_key": api_key, "action": "setStatus", "status": "8", "id": number_id}
                response = requests.get(
                    "https://smshub.org/stubs/handler_api.php", params=params, timeout=15
                )
                return response.status_code == 200 and response.text.startswith("ACCESS_")
        ...
        try:
            ...
            if loop.is_running():
                ...
                return loop.run_until_complete(
                    self.get_sms_code_async(provider, number_id, api_key)
                )
            else:
                return loop.run_until_complete(
                    self.get_sms_code_async(provider, number_id, api_key)
            ...
            try:
                ...
                if attempt > 1:
                    ...
                    if progress_callback:
                        progress_callback(
                            f"Waiting {actual_delay:.1f}s before attempt {attempt}/{max_attempts}"
                        )
                ...
                if progress_callback:
                    progress_callback(
                        f"Checking SMS from {provider} (attempt {attempt}/{max_attempts})"
                    )

                # Attempt to retrieve SMS code (call in executor to not block)
                loop = asyncio.get_event_loop()
                code = await loop.run_in_executor(
                    None, self._get_sms_code_sync, provider, number_id, api_key
                )
                ...
                logger.debug(
                    f"SMS not ready yet from {provider} (attempt {attempt}/{max_attempts})"
                )
                ...
                logger.debug(
                    f"{provider} SMS check connection error on attempt {attempt}, will retry..."
                )
    ...
    def _get_sms_activate_sms_code(self, activation_id: str, api_key: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.get(
                "https://api.sms-activate.org/stubs/handler_api.php", params=params, timeout=30
            )
    ...
    def _get_daisysms_number(self, country: str, api_key: str, config: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            data = {"service": config["service_code"], "country": country.upper()}
...
class AccountCreator:
    ...
    def validate_bulk_run_preflight(
        ...
        is_valid, capability_error = self.phone_provider.validate_provider_capability(
            provider, country
        )
        if not is_valid:
            errors.append(f"Provider capability: {capability_error}")
            return {"success": False, "errors": errors, "warnings": warnings, "can_proceed": False}
        ...
        try:
            inventory_result = self.phone_provider.check_inventory(
                provider, country, api_key, requested_count
            )

            if not inventory_result.get("success"):
                errors.append(
                    f"Inventory check failed: {inventory_result.get('error', 'Unknown error')}"
                )
            else:
    ...
    async def create_account_with_concurrency(
        self, config: Dict[str, Any], progress_callback: Optional[callable] = None
    ) -> Dict[str, Any]:
        """
        ...
        if pool:
            try:
                ...
                if proxy:
                    ...
                    logger.info(
                        f"Assigned proxy {proxy.proxy_key} to account {phone_number} from pool"
                    )
    ...
    async def _release_proxy_assignment(
        self, assigned_proxy: Dict, phone_number: Optional[str], temp_phone_id: str
    ):
    ...
    async def _record_proxy_failure(
        self, assigned_proxy: Optional[Dict], phone_number: Optional[str]
    ):
    ...
    def _validate_provider_config(
        self, provider: str, country: str, api_key: Optional[str]
        ...
        for i in range(count):
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    self._notify_progress(
                        i + 1, count, f"Account {i+1} failed: {result.get('error')}"
                    )
            ...
            if i < count - 1 and self.creation_active:
                min_delay, max_delay = self._get_delay_range(
                    config, config.get("realistic_timing", False)
                )
    ...
    async def create_new_account(self, config: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            phone_data = await asyncio.to_thread(
                self.phone_provider.get_number, provider, country, api_key
            )
            if not phone_data:
                return {
                    "success": False,
            ...
            if assigned_proxy:
                ...
                self.account_proxies[phone_number] = self.account_proxies.pop(
                    temp_phone_id, assigned_proxy
                )
            ...
            try:
                ...
                await self._record_proxy_failure(assigned_proxy, phone_number)
                return {"success": False, "error": translate_error(e, "sending verification code")}
            except Exception as e:
                await self._record_proxy_failure(assigned_proxy, phone_number)
                return {"success": False, "error": translate_error(e, "requesting SMS code")}
            ...
            try:
                ...
                await self._record_proxy_failure(assigned_proxy, phone_number)
                return {"success": False, "error": translate_error(e, "signing in")}
            except PhoneCodeInvalid as e:
                await self._record_proxy_failure(assigned_proxy, phone_number)
                return {"success": False, "error": translate_error(e, "verifying code")}
            ...
            return {"success": False, "error": translate_error(e, "account creation")}
        finally:
    ...
    def _generate_device_fingerprint(self, config: Dict) -> Dict:
        ...
        return {
            ...
            "system_lang_code": config.get("country", "en").lower()
            + "-"
            + config.get("country", "US").upper(),
        }

    async def _create_client_without_start(
        self,
        session_id: str,
        session_name: str,
        phone_number: str,
        ...
        try:
            ...
            if not api_id or not api_hash:
                logger.error(
                    "Missing Telegram API credentials. Set in config or environment variables."
                )
                return None
    ...
    async def _handle_sms_with_anti_detection(
        self, session_id: str, phone_data: Dict, config: Dict
    ) -> Optional[str]:
        ...
        for _attempt in range(max_sms_attempts):
            ...
            if sms_code:
                ...
                typing_delays = self.anti_detection.simulate_human_typing(
                    session_id, sms_code, "code_entry"
                )
    ...
    async def _setup_profile_with_anti_detection(
        self, session_id: str, client: Client, config: Dict, fingerprint: Dict
    ):
        ...
        try:
            ...
            if config.get("profile_photo"):
                ...
                try:
                    ...
                    await self._set_profile_photo_stealth(
                        client, config["profile_photo"], allow_skip=True
                    )
    ...
    async def _set_profile_photo_stealth(
        self, client: Client, photo_path: str, allow_skip: bool = False
    ):
        ...
        if photo_file.suffix.lower() not in valid_extensions:
            raise ValueError(
                f"Invalid photo format: {photo_file.suffix}. Must be one of {valid_extensions}"
            )
        ...
        try:
            ...
            with Image.open(photo_file) as img:
                ...
                if width < 160 or height < 160:
                    logger.warning(
                        f"Photo dimensions too small: {width}x{height} (min 160x160 recommended)"
                    )
                if width > 5000 or height > 5000:
                    raise ValueError(
                        f"Photo dimensions too large: {width}x{height} (max 5000x5000)"
                    )
        except ImportError:
    ...
    async def cancel_all_active_sessions(self):
        ...
        logger.info(f"✅ Cancellation complete: {success_count} successful, {error_count} errors")
