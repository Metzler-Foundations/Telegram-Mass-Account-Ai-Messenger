...
try:
    from anti_detection.anti_detection_system import (
        EnhancedAntiDetectionSystem, AccountRiskMetrics, BanRiskLevel,
        QuarantineReason, TelegramFingerprintGenerator, TelegramClientType
    )
...
class MemoryMonitor:
    ...
    def is_memory_critical(self) -> bool:
        ...
        return (
            stats['system_percent'] > self.max_memory_percent or
            stats['process_mb'] > self.cleanup_threshold_mb
        )
...
class AccountManager:
    ...
    def __init__(self, db: MemberDatabase, warmup_service=None):
        ...
        # Batch updates
        self.batch_updater = BatchStatusUpdater(
            flush_interval=self.BATCH_UPDATE_INTERVAL
        )
    ...
    async def get_proxy_pool_manager(self):
        ...
        if self._proxy_pool_manager is None:
            async with self._init_lock:
                if self._proxy_pool_manager is None:
                    try:
                        from proxy_pool_manager import get_proxy_pool_manager, init_proxy_pool_manager
    ...
    async def _run_health_checks(self):
        ...
        for phone in phones_to_check:
            try:
                ...
                # Update status
                if is_connected:
                    await self.batch_updater.queue_update(phone, {
                        'is_online': True,
    ...
    async def start_client(self, phone_number: str, force_new: bool = False) -> bool:
        ...
        try:
            ...
            if success:
                ...
                if account_data.get('one_time_phone', True) or account_data.get('always_online', True):
    ...
    async def _evict_lower_priority_client(self, high_priority_phone: str) -> bool:
        ...
        for phone in list(self.active_clients.keys()):
            ...
            client_idx = priority_order.index(client_priority) if client_priority in priority_order else 1
    ...
    async def _setup_voice_service(self, phone_number: str, client: TelegramClient, account_data: Dict):
    ...
    async def _setup_auto_reply(self, phone_number: str, client: TelegramClient, account_data: Dict):
    ...
    async def stop_client(self, phone_number: str, force: bool = False):
        ...
        if phone_number in self.active_clients:
            ...
            # Update shard
            if shard_id in self.shards:
                self.shards[shard_id].active_connections = max(0, self.shards[shard_id].active_connections - 1)
    ...
    def get_account_list(self) -> List[Dict]:
        ...
        for phone_number, account_data in self.accounts.items():
            ...
            voice_enabled = voice_config.get('enabled', False) if isinstance(voice_config, dict) else False
            ...
            account_list.append({
                ...
                'has_custom_brain': not account_data.get('brain_config', {}).get('use_shared_brain', True)
            })
    ...
    def get_shard_stats(self) -> Dict[str, Dict]:
        ...
        for shard_id, shard in self.shards.items():
            stats[shard_id] = {
                ...
                'utilization': (shard.active_connections / shard.max_connections * 100) if shard.max_connections > 0 else 0
            }
        return stats
    ...
    def update_account_status(self, phone_number: str, status: AccountStatus, metadata: Optional[Dict] = None):
        ...
        old_status = AccountStatus(self.account_status[phone_number].get('status', AccountStatus.CREATED.value))
        new_status = status
    ...
    def _is_valid_status_transition(self, old_status: AccountStatus, new_status: AccountStatus) -> bool:
        """Validate if status transition is allowed."""
        valid_transitions = {
            AccountStatus.CREATED: [AccountStatus.CLONING, AccountStatus.WARMING_UP, AccountStatus.READY, AccountStatus.ERROR, AccountStatus.SUSPENDED],
            AccountStatus.CLONING: [AccountStatus.CLONED, AccountStatus.ERROR],
            AccountStatus.CLONED: [AccountStatus.WARMING_UP, AccountStatus.READY, AccountStatus.ERROR],
            AccountStatus.WARMING_UP: [AccountStatus.READY, AccountStatus.ERROR, AccountStatus.SUSPENDED],
            AccountStatus.READY: [AccountStatus.ERROR, AccountStatus.SUSPENDED, AccountStatus.BANNED],
            AccountStatus.ERROR: [AccountStatus.CREATED, AccountStatus.CLONING, AccountStatus.WARMING_UP, AccountStatus.READY],
            AccountStatus.SUSPENDED: [AccountStatus.READY, AccountStatus.ERROR, AccountStatus.BANNED],
            AccountStatus.BANNED: []
        }
        
        return new_status in valid_transitions.get(old_status, [])
    
    def _notify_status_change(self, phone_number: str, old_status: AccountStatus, 
                              new_status: AccountStatus, metadata: Dict):
    ...
    def get_online_accounts(self) -> List[str]:
        ...
        return [
            phone for phone, status in self.account_status.items()
            ...
            if result.get('success') and result.get('account'):
                ...
                if self.warmup_service:
                    try:
                        ...
                        warmup_job_id = self.warmup_service.add_warmup_job(phone_number, WarmupPriority.NORMAL)
                        self.account_status[phone_number]['warmup_job_id'] = warmup_job_id
                        self.update_account_status(phone_number, AccountStatus.WARMING_UP, {
    ...
    def get_account_stats(self) -> Dict:
        ...
        total_messages = sum(
            ...
            for m in self.account_metrics.values()
        )
    ...
    def get_accounts_by_type(self, account_type: AccountType) -> List[str]:
        ...
        return [
            phone for phone, data in self.accounts.items()
    ...
    def get_voice_enabled_accounts(self) -> List[str]:
        ...
        return [
            phone for phone in self.accounts.keys()
            if self.is_voice_enabled(phone)
        ]
    ...
    def get_full_account_config(self, phone_number: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'voice_config': self.get_voice_config(phone_number).to_dict() if self.get_voice_config(phone_number) else {},
            'status': self.account_status.get(phone_number, {}),
            'metrics': asdict(self.account_metrics.get(phone_number, AccountMetrics())),
            'shard_id': self.account_to_shard.get(phone_number),
            'basic_info': {
                k: v for k, v in self.accounts[phone_number].items()
                if k not in ['brain_config', 'voice_config', 'account_type']
            }
        }
    
    def update_account_config(self, phone_number: str, 
                              account_type: AccountType = None,
                              brain_config: BrainConfig = None,
                              voice_config = None,
                              priority: AccountPriority = None) -> bool:
    ...
    async def check_account_health(self, phone_number: str) -> Dict[str, Any]:
        ...
        if self._anti_detection_system:
            ...
            if status.get('is_quarantined'):
                ...
                result['warnings'].append(f"Account is quarantined until {status.get('quarantine_release_at')}")
        ...
        if indicators.get('flood_wait_count', 0) > 3:
            result['warnings'].append(f"Multiple flood waits detected ({indicators['flood_wait_count']})")
    ...
    def record_api_error(self, phone_number: str, error: Exception) -> Dict[str, Any]:
        ...
        if isinstance(error, FloodWait):
            ...
            self.update_account_status(phone_number, AccountStatus.ERROR, {
    ...
    def _quarantine_account(self, phone_number: str, reason: str, duration_minutes: int):
        ...
        # Update status
        self.update_account_status(phone_number, AccountStatus.SUSPENDED, {
            'quarantine_reason': reason,
            'quarantine_until': (datetime.now() + timedelta(minutes=duration_minutes)).isoformat()
        })
        ...
        if self._anti_detection_system and ENHANCED_ANTI_DETECTION_AVAILABLE:
            self._anti_detection_system.quarantine_account(
                phone_number, 
                QuarantineReason.SUSPICIOUS_ACTIVITY,
                duration_minutes
            )
    ...
    def get_account_full_status(self, phone_number: str) -> Optional[Dict]:
        ...
        return {
            ...
            'is_online': status_info.get('is_online', False),
            'connection_status': self.connection_manager.get_connection_status(phone_number) if phone_number in self.active_clients else None,
            'warmup_info': {
