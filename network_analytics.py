...
class NetworkAnalytics:
    ...
    def _save_cluster(self, cluster: NetworkCluster, members: List[int]):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (cluster.cluster_id, cluster.member_count, cluster.density,
              json.dumps(cluster.top_influencers), datetime.now(), datetime.now()))
    ...
    def get_network_stats(self) -> Dict:
        ...
        # Cluster count
        cursor.execute("SELECT COUNT(DISTINCT cluster_id) FROM network_nodes WHERE cluster_id IS NOT NULL")
    ...
    def rebuild_graph_from_db(self):
        ...
        conn.close()
        logger.info(f"Rebuilt graph: {self.graph.number_of_nodes()} nodes, {self.graph.number_of_edges()} edges")
