...
class ValidationHelper:
    ...
    def validate_phone_number(phone: str) -> Tuple[bool, str]:
        ...
        if not cleaned.startswith("+"):
            return False, "Phone number must start with '+' followed by country code. Example: +1234567890"
        
        # Check if rest is digits
        if not cleaned[1:].isdigit():
            return False, "Phone number should only contain digits after the '+'. Example: +1234567890"
    ...
    def validate_api_id(api_id: str) -> Tuple[bool, str]:
        ...
        if not api_id.isdigit():
            return False, "API ID should be a number (e.g., 12345678). Get it from https://my.telegram.org/apps"
        
        if len(api_id) < 6:
            return False, "API ID seems too short. It should be 7-8 digits. Check https://my.telegram.org/apps"
    ...
    def validate_api_hash(api_hash: str) -> Tuple[bool, str]:
        ...
        if len(api_hash) != 32:
            return False, "API Hash should be exactly 32 characters. Check https://my.telegram.org/apps"
        
        if not re.match(r'^[a-f0-9]{32}$', api_hash.lower()):
            return False, "API Hash should contain only letters (a-f) and numbers (0-9). Check https://my.telegram.org/apps"
    ...
    def validate_gemini_api_key(api_key: str) -> Tuple[bool, str]:
        ...
        if not api_key:
            return False, "Gemini API key is required for AI features. Get it from https://makersuite.google.com/app/apikey"
        
        # Gemini keys typically start with "AI" and are ~39 characters
        if not api_key.startswith("AI"):
            return False, "Gemini API key should start with 'AIza'. Get it from https://makersuite.google.com/app/apikey"
        
        if len(api_key) < 30:
            return False, "Gemini API key seems too short. Check https://makersuite.google.com/app/apikey"
...
class ProgressHelper:
    ...
    def get_message(stage: str, progress: int = 0) -> str:
        ...
        """
        base_message = ProgressHelper.STAGE_MESSAGES.get(stage, f"⚙️ {stage.replace('_', ' ').title()}...")
