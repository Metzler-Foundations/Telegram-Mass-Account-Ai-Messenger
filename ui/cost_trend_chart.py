...
class CostTrendChart(QWidget):
    ...
    def setup_ui(self):
        ...
        if MATPLOTLIB_AVAILABLE:
            ...
            no_chart_label = QLabel(
                "Cost trend charts require matplotlib.\nInstall with: pip install matplotlib"
            )
        ...
        self.total_cost_label.setStyleSheet(
            f"color: {chart_colors['text_secondary']}; font-size: 14px; font-weight: 500;"
        )
        stats_layout.addWidget(self.total_cost_label)

        self.avg_daily_label = QLabel("Avg Daily: $0.00")
        self.avg_daily_label.setStyleSheet(
            f"color: {chart_colors['text_secondary']}; font-size: 14px; font-weight: 500;"
        )
    ...
    def refresh_chart(self):
        ...
        try:
            ...
            self.trend_label.setStyleSheet(
                f"color: {trend_color}; font-size: 14px; font-weight: 500;"
            )
            ...
            # Plot line
            ax.plot(
                dates, costs, color=chart_colors["accent"], linewidth=2, marker="o", markersize=4
            )
            ...
            ax.set_title(
                "Cost Trend Over Time", color=chart_colors["text"], fontsize=14, fontweight="bold"
            )
