...
class UIController(QObject):
    ...
    def get_campaigns_overview(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for campaign in campaigns:
                optimization = self.business_logic.campaign_service.optimize_campaign_targeting(
                    campaign["id"]
                )
        ...
        try:
            ...
            if success:
                ...
                self.error_occurred.emit(
                    "account_update", f"Failed to update account {phone_number}"
                )
    ...
    # Member Operations
    def get_members_for_campaign_ui(
        self, channel_id: str, criteria: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Get members for campaign with business logic filtering."""
        try:
            members = self.business_logic.member_service.filter_members_for_campaign(
                channel_id, criteria
            )
            metrics = self.business_logic.member_service.calculate_campaign_metrics(members)

            return {"success": True, "members": members, "metrics": metrics, "count": len(members)}

        except Exception as e:
            logger.error(f"Error getting members for campaign: {e}")
            return {"success": False, "error": str(e), "members": [], "metrics": {}, "count": 0}
