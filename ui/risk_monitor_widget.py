...
try:
    from monitoring.account_risk_monitor import AccountRiskMonitor, RiskLevel, get_risk_monitor
...
class RiskMonitorWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.risk_filter.addItems(
            ["All", "Safe", "Low", "Medium", "High", "Critical", "Quarantine"]
        )
        ...
        self.loading_overlay = (
            LoadingOverlay(self.accounts_table, "Refreshing risk data…") if LoadingOverlay else None
        )
    ...
    def _render_accounts(self, accounts):
        ...
        for row, account in enumerate(accounts):
            ...
            flood_chip.setProperty(
                "state", self._severity_state(account.total_floodwaits_24h, [0, 1, 3])
            )
            ...
            error_chip.setProperty(
                "state", self._severity_state(account.total_errors_24h, [0, 5, 20])
            )
