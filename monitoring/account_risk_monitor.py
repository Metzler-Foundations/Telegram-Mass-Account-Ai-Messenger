...
class AccountRiskMonitor:
    ...
    def calculate_risk_score(
        ...
        if floodwait_score >= 75:
            recommendations.append("🚨 Multiple FloodWaits detected - pause all campaigns on this account")
            should_quarantine = True
        elif floodwait_score >= 50:
            recommendations.append("⚠️ FloodWait activity elevated - reduce message frequency by 50%")
    ...
    def get_high_risk_accounts(self, min_risk_level: RiskLevel = RiskLevel.HIGH) -> List[AccountRiskScore]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                import json
                for row in cursor:
                    accounts.append(AccountRiskScore(
                        phone_number=row['phone_number'],
                        overall_score=row['overall_score'],
                        risk_level=RiskLevel(row['risk_level']),
                        floodwait_score=row['floodwait_score'],
    ...
    def get_risk_summary(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor = conn.execute('SELECT COUNT(*) as count FROM account_risk_scores WHERE should_quarantine = 1')
