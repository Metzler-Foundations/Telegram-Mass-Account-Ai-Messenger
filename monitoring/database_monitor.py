...
class DatabaseMonitor:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {"timestamp": datetime.now().isoformat(), "databases": {}}
    ...
    def get_total_size_mb(self) -> float:
        ...
        total = sum(m["size_mb"] for m in self.metrics.get("databases", {}).values())
        return round(total, 2)

    def should_alert(self) -> bool:
        """Check if any database needs attention."""
        return any(m.get("alert", False) for m in self.metrics.get("databases", {}).values())
